  ...
  async confirmOrPreview(
    ...
    if (!request.confirmation) {
      ...
      if (!stored) {
        throw createConnectorError({
          code: "CONFLICT",
          message: "Die Inhaltsvorschau konnte nicht atomar gespeichert werden.",
          request_id: request.mutation.context.request_id,
          retryable: true,
        });
      }
...
export class ContentJobPolicy {
  requiresJob(request: K12MutationRequest): boolean {
    return request.operation.execution_gate === "job"
      || request.operation.execution_gate === "confirmed_job";
  }
}
