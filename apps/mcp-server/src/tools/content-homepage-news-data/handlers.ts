...
add("cai.news.01.list", "private", async (input, context, client) => boundedContentList(assertClub(await request(client, context, "GET", "content", `/news/club/${string(input, "club_id")}`), input, context), Number(input.limit), (entry) => minimizeNews(entry, false), Number(input.offset)));
