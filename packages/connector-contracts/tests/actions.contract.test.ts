...
describe("Comvenio connector inventory contract", () => {
  ...
  test("covers all 303 legacy actions minus the retired ones plus additive procurement and screenshot by an executable adapter or exact replacement", () => {
    ...
    // +1 budget-organigramm-04 (cai.finance.36.budget_organigram).
    expect(directActionIds).toHaveLength(373);
    expect(new Set(directActionIds).size).toBe(373);
    ...
    expect(summary).toMatchObject({
      discovered_actions: 373,
      published_domain_actions: 371,
      blocked_action_ids: [
        "cai.club.01.info",
