...
2. Fordere eine private Aktion an. ChatGPT muss OAuth Authorization Code mit PKCE S256 starten.
...
5. Entziehe im Backend testweise eine Berechtigung und wiederhole einen bereits sichtbaren Aufruf. Der aktuelle Backend-RBAC-Recheck muss sicher ablehnen und darf keine Mutation ausführen.
6. Nutze das `manager`-Konto für eine kritische synthetische Aktion. Prüfe maskierte Vorschau, Verein, Ziel, Wirkung, Anzahl und Ablaufzeit. Die zweite Bestätigung darf nur Vorschau-ID, Token und Idempotenzschlüssel senden.
7. Widerrufe den Grant. Der nächste private Aufruf muss erneut eine Verbindung verlangen.
8. Öffne Event-, Mitglieder-, Buchungs-, News- und Bestätigungs-Widget jeweils auf ChatGPT Web und Mobile. Fachvertrag und erlaubte Aktionen müssen identisch sein; Layout, Fokus und Touchziele müssen funktionieren.
9. Führe jeden Fall aus `tool-test-plan.json` aus und vergleiche die Antwort mit der benannten synthetischen Fixture.
...
- Abgelaufene oder veränderte Vorschau: neuer Preview-Lauf erforderlich.
