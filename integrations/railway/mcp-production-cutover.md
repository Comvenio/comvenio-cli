...
MCP_CIMD_CLIENT_PINS_JSON=<CIMD-Pin mit allowed_scopes ["club.read","task.read","task.write"]>
```
...
OAUTH_CIMD_CLIENT_PINS_JSON=<derselbe CIMD-Pin mit allowed_scopes ["club.read","task.read","task.write"]>
```
...
6. Im ChatGPT-Connector `club.read`, `task.read` und `task.write` als
   Standard-Scopes hinterlegen. `task.write` ist im veröffentlichten Umfang
   ausschließlich für die persönliche Aufgaben-Erinnerung vorgesehen. Nach
   einer Scope-Erweiterung die bestehende Verbindung trennen und erneut
   autorisieren.
