...
export function runAnthropicSubmissionPreflight(input: {
  ...
  const checks = [
    ...
    check("CAROUSEL_SCREENSHOTS", input.manifest.screenshots.length >= 3 && input.manifest.screenshots.length <= 5 && input.manifest.screenshots.every((item) => item.format === "png" && item.app_response_only && item.synthetic_data_only && item.prompt.trim().length > 0 && validCarouselPng(input.artifact_root, item.path)), "Drei bis fünf synthetische App-Antwort-PNGs mit separatem Prompt und mindestens 1000 Pixel Breite sind erforderlich."),
